...
public final class AutoWalkConfig {
    ...
    private static final File CONFIG_FILE = new File(FabricLoader.getInstance().getConfigDir().toFile(), "duperautowalk.json");
    ...
    private static final class AutoWalkConfigScreen extends Screen {
        ...
        private static final Identifier TEXTURE = Identifier.of(DuperAutoWalk.MOD_ID, "textures/gui/autowalk.png");
        ...
        protected void init() {
            ...
            feedbackButton = addDrawableChild(ButtonWidget.builder(feedbackText(), button -> {
            ...
            positionButton = addDrawableChild(ButtonWidget.builder(positionText(), button -> {
                setPosition(next(position));
                syncPreviewWithPosition();
                button.setMessage(positionText());
            }).dimensions(centerX - buttonWidth / 2, startY + 24, buttonWidth, buttonHeight).build());

            sizeButton = addDrawableChild(ButtonWidget.builder(sizeText(), button -> {
                setSize(next(size));
                syncPreviewWithPosition();
                button.setMessage(sizeText());
            }).dimensions(centerX - buttonWidth / 2, startY + 48, buttonWidth, buttonHeight).build());

            addDrawableChild(ButtonWidget.builder(Text.translatable("gui.done"), button -> close())
                    .dimensions(centerX - buttonWidth / 2, startY + 72, buttonWidth, buttonHeight)
                    .build());
        }
        ...
        public void render(DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            context.drawCenteredTextWithShadow(textRenderer, Text.translatable("screen.duperautowalk.config.drag_help"), this.width / 2, this.height - 112, 0xFFAAAAAA);
            context.drawCenteredTextWithShadow(textRenderer, Text.translatable("screen.duperautowalk.config.coords", previewX, previewY), this.width / 2, this.height - 102, 0xFFCCCCCC);
        }

        //? if >=1.21.9 {
        /*@Override
        public boolean mouseClicked(Click click, boolean doubled) {
            if (click.button() == 1 && isInsidePreview(click.x(), click.y())) {
                setHudFeedback(!hudFeedback);
                return true;
            }
            if (click.button() == 0 && isInsidePreview(click.x(), click.y())) {
                if (isInsideResizeHandle(click.x(), click.y())) {
                    startResize(click.x(), click.y());
                    return true;
                }
                dragging = true;
        ...
        *///?} else {
        ...
        public boolean mouseDragged(Click click, double deltaX, double deltaY) {
            if (resizing && click.button() == 0) {
                resizePreview(click.x(), click.y());
                return true;
            }
            if (dragging && click.button() == 0) {
                movePreview((int) click.x() - dragOffsetX, (int) click.y() - dragOffsetY, !isShiftDown(click.modifiers()));
                return true;
            }
        ...
        *///?} else {
        ...
        public boolean keyPressed(KeyInput input) {
            ...
            int step = (input.modifiers() & GLFW.GLFW_MOD_SHIFT) != 0 ? 5 : 1;
        ...
        *///?} else {
        ...
        private void drawPreview(DrawContext context, int mouseX, int mouseY) {
            ...
            context.drawTexture(RenderPipelines.GUI_TEXTURED, TEXTURE, previewX, previewY, 0, 0, sizePixels, sizePixels, 16, 16, 16, 16, hudFeedback ? 0xFFFFFFFF : 0x75FFFFFF);
        ...
        private Text feedbackText() {
            return Text.translatable("screen.duperautowalk.config.chat_feedback", Text.translatable(chatFeedback ? "duperautowalk.toggle.on" : "duperautowalk.toggle.off"));
        }

        private Text positionText() {
            return Text.translatable("screen.duperautowalk.config.position", Text.translatable("duperautowalk.position." + position.name().toLowerCase()));
        }

        private Text sizeText() {
            return Text.translatable("screen.duperautowalk.config.size", iconSize, iconSize);
        }
